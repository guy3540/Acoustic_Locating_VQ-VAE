...
def run_location_training():
    ...
    vae_model = torch.load("/home/guy/PycharmProjects/Acoustic_Locating_VQ-VAE/models/model_echoed_speech_6500.pt").to(device)
...
def train_location(combined_model: EchoedSpeechReconModel, location_model, optimizer, num_training_updates, train_loader,
    ...
    for i in xrange(num_training_updates):
        ...
        if (i + 1) % 50 == 0:
            ...
            plt.show()
        if (i + 1) % 1000 == 0:
            torch.save(location_model, '../models/model_location_echoed_6500_new' + str(i + 1) + '.pt')
